...
const string PLUGIN_NAME("Import image as doubleVector");
...
const char *paramHelp[] = {
	...
	HTML_HELP_OPEN()
		...
		HTML_HELP_BODY()
		"Name of the doubleVectorProperty where the feature will be stored."
		HTML_HELP_CLOSE(),
};
}

class ImportImageAsDoubleVector: public ImportModule {
public:
	PLUGININFORMATIONS(PLUGIN_NAME, "Cyrille Faucheux", "2013-08-08", "", "1.0", "File")

	ImportImageAsDoubleVector(PluginContext *context) :
	...
	bool importGraph()
	{
		try {
			...
			tlp::DoubleVectorProperty *featureProperty = graph->getProperty< tlp::DoubleVectorProperty >(property_name);

			importVectorFeature< ImageType, tlp::DoubleVectorProperty >(image, graph, featureProperty, pluginProgress);
...
PLUGIN(ImportImageAsDoubleVector);
