	...
	const char
		* paramHelp[] = {
			...
			HTML_HELP_OPEN()
				...
				HTML_HELP_BODY()
				"The IntegerProperty where the data will be stored."
				HTML_HELP_CLOSE(),
		};
}

class LoadIntegerPropertyFromImage: public tlp::Algorithm {
	std::string file;
	tlp::IntegerProperty *property;

public:
	PLUGININFORMATIONS("Load integer property from image", "Cyrille FAUCHEUX", "2013-08-01", "", "1.0", "Image")

	LoadIntegerPropertyFromImage(const tlp::PluginContext *context):
		tlp::Algorithm(context)
	{
		addInParameter< std::string >         ("file::Image", paramHelp[0], "");
		addInParameter< tlp::IntegerProperty >("Property",    paramHelp[1], "data");
	}
	...
	bool run() {
		try {
			...
			importFeature< ImageType, tlp::IntegerProperty >(image, graph, property, pluginProgress);
		} catch(std::runtime_error &ex) {
			if(pluginProgress)
...
PLUGIN(LoadIntegerPropertyFromImage)
