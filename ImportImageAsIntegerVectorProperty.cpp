...
const string PLUGIN_NAME("Import image as integerVector");
...
const char *paramHelp[] = {
	...
	HTML_HELP_OPEN()
		...
		HTML_HELP_BODY()
		"Name of the integerVectorProperty where the feature will be stored."
		HTML_HELP_CLOSE(),
};
}

class ImportImageAsIntegerVector: public ImportModule {
public:
	PLUGININFORMATIONS(PLUGIN_NAME, "Cyrille Faucheux", "2013-08-08", "", "1.0", "File")

	ImportImageAsIntegerVector(PluginContext *context) :
	...
	bool importGraph()
	{
		try {
			...
			tlp::IntegerVectorProperty *featureProperty = graph->getProperty< tlp::IntegerVectorProperty >(property_name);

			importVectorFeature< ImageType, tlp::IntegerVectorProperty >(image, graph, featureProperty, pluginProgress);
...
PLUGIN(ImportImageAsIntegerVector);
