...
const string PLUGIN_NAME("Import image as double");
...
const char *paramHelp[] = {
	...
	HTML_HELP_OPEN()
		...
		HTML_HELP_BODY()
		"Name of the doubleProperty where the feature will be stored."
		HTML_HELP_CLOSE(),
};
}

class ImportImageAsDouble: public ImportModule {
public:
	PLUGININFORMATIONS(PLUGIN_NAME, "Cyrille Faucheux", "2013-08-08", "", "1.0", "File")

	ImportImageAsDouble(PluginContext *context) :
	...
	bool importGraph()
	{
		try {
			...
			tlp::DoubleProperty *featureProperty = graph->getProperty< tlp::DoubleProperty >(property_name);

			importFeature< ImageType, tlp::DoubleProperty >(image, graph, featureProperty, pluginProgress);
