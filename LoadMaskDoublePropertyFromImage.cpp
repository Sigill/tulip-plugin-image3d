...
class LoadMaskAsDoublePropertyFromImage: public tlp::Algorithm {
	std::string file;
	tlp::DoubleProperty *property;

public:
	PLUGININFORMATIONS("Load mask as double property from image", "Cyrille FAUCHEUX", "2013-08-01", "", "1.0", "Image")

	LoadMaskAsDoublePropertyFromImage(const tlp::PluginContext *context):
	...
	~LoadMaskAsDoublePropertyFromImage() {}
	...
	bool run() {
		try {
			...
			importSelectionAsNumericProperty< ImageType, tlp::DoubleProperty >(image, graph, property, pluginProgress);
		} catch(std::runtime_error &ex) {
			if(pluginProgress)
...
PLUGIN(LoadMaskAsDoublePropertyFromImage)
